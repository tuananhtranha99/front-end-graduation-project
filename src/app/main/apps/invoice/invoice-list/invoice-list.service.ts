...
export class InvoiceListService implements Resolve<any> {
  ...
  public getOrderList(){
    return this._httpClient.get<any>(
      "http://localhost:8080/purchase-order/find-order"
    );
  }

  public updateOrderStatus(params){
    return this._httpClient.post<any>(
      "http://localhost:8080/purchase-order/update-status", {...params}
    ).pipe();
  }

  public deleteOrder(orderId){
    return this._httpClient.delete<any>(
      `http://localhost:8080/purchase-order?orderId=${orderId}`
    ).pipe();
  }
}
